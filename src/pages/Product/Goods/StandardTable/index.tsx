...
import { TableListItem } from '@/pages/Product/Goods/data';
...
class StandardTable extends Component<StandardTableProps<TableListItem>, StandardTableState> {
  ...
  render() {
    ...
    const expandedRowRender = (record: TableListItem) => {
      const columns = [
        { title: 'Product Code', dataIndex: 'Product Code', key: 'Product Code' },
      ...
      // const optionContents = JSON.parse(record.optionContents || '[]');
      // optionContents.forEach(item => {
      //   record.optionContents
      // });
      return (
        <Table
          rowKey="Product Code"
          columns={columns}
          dataSource={JSON.parse(record.optionContents || '[]')}
          pagination={false}
        />
      );
    };
